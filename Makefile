...
offline:
	...
	@docker rm -f staticfile &> /dev/null
	@docker run -d --name staticfile -p 8081:8081 --restart always -v /tmp/staticfile:/data docker.io/buxiaomo/staticfile:1.0 &> /dev/null
